...
| 使用 AI 功能 | 管理员后台 → AI 配置 | 每日一签 AI 需配置 DeepSeek API Key；AI 任务分析支持 DeepSeek / OpenAI API Key。公网多人统一 AI 需要后端 AI Proxy。 |
...
- 每日一签：传统签文无需 API Key；AI 签文需要在管理员后台配置 DeepSeek API Key。
- AI 辅助：任务分析、智能建议等能力，需要在管理员后台配置 DeepSeek 或 OpenAI API Key。
- 数据导入/导出：用于本地备份和迁移。
...
| 配置 AI 功能 | 管理员后台 | 每日一签 AI 使用 DeepSeek；AI 任务分析支持 DeepSeek / OpenAI。均采用 BYOK，需要自行配置 API Key。 |
...
配置路径：登录管理员后台 → AI 签语配置 / AI 服务配置 → 填写对应服务商 API Key → 保存并启用。
